...
BACKUP_ROOT="/var/backups/vulcano"
RETENTION_HOURS=24
...
BACKUP_ROOT="${BACKUP_ROOT:-/var/backups/vulcano}"
...
MYSQL_DATABASE="${MYSQL_DATABASE:-vulcano}"
MYSQL_USER="${MYSQL_USER:-root}"
MYSQL_PASSWORD=
# App (para storage)
APP_CONTAINER="${APP_CONTAINER:-infra-app-vulcano_app.1.$(docker service ps infra-app-vulcano_app -q --no-trunc 2>/dev/null | head -1)}"
...
ARCHIVE_NAME="vulcano_backup_${TIMESTAMP}.tar.gz"
...
if $R2_OK && command -v rclone &>/dev/null; then
  ...
  if rclone copyto "$ARCHIVE_PATH" "s3:${R2_BUCKET}/vulcano/${ARCHIVE_NAME}" \
      --s3-no-check-bucket \
...
find "$BACKUP_ROOT" -maxdepth 1 -name 'vulcano_backup_*.tar.gz' -mmin "+$((RETENTION_HOURS * 60))" -print -delete \
...
LOCAL_COUNT=$(find "$BACKUP_ROOT" -maxdepth 1 -name 'vulcano_backup_*.tar.gz' | wc -l)
