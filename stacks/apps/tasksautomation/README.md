...
Como a stack usa imagem local fixa (`local/tasksautomation-app:latest`), o `docker stack deploy` pode ignorar uma digest nova quando a spec do serviço não muda. Para evitar isso sem abrir uma segunda janela de rollout, os serviços recebem a variável `APP_BUILD_COMMIT=${APP_GIT_COMMIT}` renderizada no `docker-stack.yml`: cada deploy muda a spec do Swarm e força a atualização normal da stack. Assim o `app` continua a usar o rolling update definido em `deploy.update_config` (`start-first`) e o websocket também reinicia pelo próprio deploy, sem `docker service update --force` extra.
