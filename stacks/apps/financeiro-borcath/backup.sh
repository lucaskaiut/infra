#!/usr/bin/env bash
# =============================================================================
# Financeiro Borcath Backup Script
...
BACKUP_ROOT="/home/kaiut/backups/financeiro-borcath"
RETENTION_HOURS=24
...
BACKUP_ROOT="${BACKUP_ROOT:-/home/kaiut/backups/financeiro-borcath}"
...
MYSQL_DATABASE="${MYSQL_DATABASE:-financeiro_borcath}"
...
ARCHIVE_NAME="financeiro_borcath_backup_${TIMESTAMP}.tar.gz"
...
if $R2_OK && command -v rclone &>/dev/null; then
  ...
  if rclone copyto "$ARCHIVE_PATH" "s3:${R2_BUCKET}/financeiro-borcath/${ARCHIVE_NAME}" \
      --s3-no-check-bucket \
  ...
  TO_DELETE=$(rclone delete "s3:${R2_BUCKET}/financeiro-borcath/" \
    --s3-no-check-bucket \
  ...
  if [[ -n "$TO_DELETE" ]]; then
    ...
    rclone delete "s3:${R2_BUCKET}/financeiro-borcath/" \
      --s3-no-check-bucket \
...
find "$BACKUP_ROOT" -maxdepth 1 -name 'financeiro_borcath_backup_*.tar.gz' -mmin "+$((RETENTION_HOURS * 60))" -print -delete \
...
LOCAL_COUNT=$(find "$BACKUP_ROOT" -maxdepth 1 -name 'financeiro_borcath_backup_*.tar.gz' | wc -l)
