...
BACKUP_ROOT="/home/kaiut/backups/monensure"
RETENTION_HOURS=24
...
BACKUP_ROOT="${BACKUP_ROOT:-/home/kaiut/backups/monensure}"
...
MYSQL_DATABASE="${MYSQL_DATABASE:-monensure}"
MYSQL_USER="${MYSQL_USER:-root}"
MYSQL_PASSWORD=
# App (para storage)
APP_CONTAINER="${APP_CONTAINER:-$(docker ps --filter name=infra-app-monensure_app --format '{{.Names}}' 2>/dev/null | head -1)}"
...
ARCHIVE_NAME="monensure_backup_${TIMESTAMP}.tar.gz"
...
if $R2_OK && command -v rclone &>/dev/null; then
  ...
  if rclone copyto "$ARCHIVE_PATH" "s3:${R2_BUCKET}/monensure/${ARCHIVE_NAME}" \
      --s3-no-check-bucket \
  ...
  TO_DELETE=$(rclone delete "s3:${R2_BUCKET}/monensure/" \
    --s3-no-check-bucket \
  ...
  if [[ -n "$TO_DELETE" ]]; then
    ...
    rclone delete "s3:${R2_BUCKET}/monensure/" \
      --s3-no-check-bucket \
...
find "$BACKUP_ROOT" -maxdepth 1 -name 'monensure_backup_*.tar.gz' -mmin "+$((RETENTION_HOURS * 60))" -print -delete \
...
LOCAL_COUNT=$(find "$BACKUP_ROOT" -maxdepth 1 -name 'monensure_backup_*.tar.gz' | wc -l)
