...
| **deploy-vulcano-webhook** | Seed `ci/jenkins/seed-deploy-vulcano-webhook-job.groovy`, ou `./ci/jenkins/create-webhook-job-from-template.sh` (ver `stacks/apps/vulcano/README.md`) | Webhook GitHub: push em `main` no repo **vulcano** com alterações em **`api/`**; `./ci/deploy-app.sh vulcano`; credencial **`vulcano-webhook-token`**. Deploy key SSH em `stacks/jenkins/keys-vulcano/`. API em `sistema-api.<DOMAIN>`. |
