...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub atena (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'atena-webhook-token',
      printContributedVariables: true,
      printPostContent: false
    )
  }
  stages {
    stage('Filtro branch / backend / repo') {
      steps {
        script {
          env.RUN_DEPLOY = '0'
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/atena') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/atena.git' '${before}' '${after}' backend
                exit \$?
              """,
              returnStatus: true
            )
            ...
            if (status == 1) {
              currentBuild.result = 'NOT_BUILT'
              echo 'Sem alterações em backend/ neste push (validação por before/after).'
              return
            }
            currentBuild.result = 'NOT_BUILT'
              ...
              try {
                ...
                for (c in commits) {
                  ...
                  for (p in paths) {
                    ...
                    def s = p.toString()
                    if (s == 'backend' || s.startsWith('backend/')) {
                      return true
                    }
                  }
                }
          ...
          if (!touch.call(env.COMMITS_PAYLOAD)) {
            currentBuild.result = 'NOT_BUILT'
            echo 'Sem alterações relevantes em backend/ e sem par before/after no payload; deploy não necessário.'
            return
          }
          env.RUN_DEPLOY = '1'
        }
    ...
    stage('Deploy Atena') {
      ...
      steps {
        ...
        sh """
          ...
          export NOTIFY_APP_SLUG=atena
          ./ci/deploy-app.sh atena
        """
      }
      post {
        always {
