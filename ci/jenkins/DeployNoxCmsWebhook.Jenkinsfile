...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub nox-cms (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'nox-cms-webhook-token',
    ...
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/nox-cms') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/nox-cms.git' '${before}' '${after}' api
                exit \$?
              """,
              returnStatus: true
            )
    ...
    stage('Deploy nox-cms') {
      ...
      steps {
        ...
        sh """
          ...
          export NOTIFY_APP_SLUG=nox-cms
          ./ci/deploy-app.sh nox-cms
        """
      }
      post {
        always {
