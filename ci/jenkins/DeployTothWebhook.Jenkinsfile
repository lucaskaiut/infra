...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub toth (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'toth-webhook-token',
    ...
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/toth') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/toth.git' '${before}' '${after}' api
                exit \$?
              """,
              returnStatus: true
            )
    ...
    stage('Deploy Toth CRM') {
      ...
      steps {
        ...
        sh """
          ...
          export NOTIFY_APP_SLUG=toth
          ./ci/deploy-app.sh toth
        """
      }
      post {
        always {
