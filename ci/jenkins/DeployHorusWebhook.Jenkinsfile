...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub horus (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'horus-webhook-token',
    ...
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/horus') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/horus.git' '${before}' '${after}' api
                exit \$?
              """,
              returnStatus: true
            )
    ...
    stage('Deploy Horus') {
      ...
      steps {
        ...
        sh """
          ...
          export NOTIFY_APP_SLUG=horus
          ./ci/deploy-app.sh horus
        """
      }
      post {
        always {
