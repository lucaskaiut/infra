...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub vulcano (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'vulcano-webhook-token',
    ...
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/vulcano') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/vulcano.git' '${before}' '${after}' api
                exit \$?
              """,
              returnStatus: true
            )
    ...
    stage('Deploy Vulcano') {
      ...
      steps {
        ...
        sh """
          ...
          export NOTIFY_APP_SLUG=vulcano
