...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub plutao (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'plutao-webhook-token',
    ...
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/plutao') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/plutao.git' '${before}' '${after}' api
                exit \$?
              """,
              returnStatus: true
            )
