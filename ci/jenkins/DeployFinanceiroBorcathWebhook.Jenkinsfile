...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub financeiro-borcath (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'financeiro-borcath-webhook-token',
  ...
  stages {
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/financeiro-borcath') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/financeiro-borcath.git' '${before}' '${after}' api
                exit \$?
              """,
              returnStatus: true
            )
    ...
    stage('Deploy Financeiro Borcath') {
      when {
      ...
      steps {
        ...
        sh """
          ...
          export NOTIFY_APP_SLUG=financeiro-borcath
          ./ci/deploy-app.sh financeiro-borcath
        """
      }
      post {
        always {
          ...
          sh """
            ...
            export APP_SLUG=financeiro-borcath
            export NOTIFY_APP_SLUG=financeiro-borcath
