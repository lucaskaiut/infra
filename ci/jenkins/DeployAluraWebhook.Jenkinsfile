...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub alura (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'alura-webhook-token',
    ...
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/alura') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/alura.git' '${before}' '${after}' api
                exit \$?
              """,
              returnStatus: true
            )
    ...
    stage('Deploy Alura') {
      ...
      steps {
        ...
        sh """
          ...
          export NOTIFY_APP_SLUG=alura
          ./ci/deploy-app.sh alura
        """
      }
      post {
        always {
