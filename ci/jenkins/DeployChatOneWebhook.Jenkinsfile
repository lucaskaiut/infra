...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub chat-one (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'chat-one-webhook-token',
    ...
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/chat-one') {
            currentBuild.result = 'NOT_BUILT'
