...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub financeiro-apolar (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'financeiro-apolar-webhook-token',
  ...
  stages {
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/financeiro-apolar') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/financeiro-apolar.git' '${before}' '${after}' api
                exit \$?
              """,
              returnStatus: true
            )
    ...
    stage('Deploy Financeiro Apolar') {
      when {
      ...
      steps {
        ...
        sh """
          ...
          export NOTIFY_APP_SLUG=financeiro-apolar
          ./ci/deploy-app.sh financeiro-apolar
        """
      }
      post {
        always {
          ...
          sh """
            ...
            export APP_SLUG=financeiro-apolar
            export NOTIFY_APP_SLUG=financeiro-apolar
