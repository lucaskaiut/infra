...
pipeline {
  ...
  triggers {
    GenericTrigger(
      causeString: 'Webhook GitHub monensure (ref=$GIT_REF)',
      genericVariables: [
      ...
      tokenCredentialId: 'monensure-webhook-token',
    ...
    stage('Filtro branch / api / repo') {
      steps {
        script {
          ...
          if (env.REPO_FULL?.trim() && env.REPO_FULL != 'lucaskaiut/monensure') {
            currentBuild.result = 'NOT_BUILT'
          ...
          if (before && after) {
            def status = sh(
              script: """
                ...
                ./ci/check-git-range-touches-path.sh 'https://github.com/lucaskaiut/monensure.git' '${before}' '${after}' api
                exit \$?
              """,
              returnStatus: true
            )
    ...
    stage('Deploy Monensure') {
      ...
      steps {
        ...
        sh """
          ...
          export NOTIFY_APP_SLUG=monensure
          ./ci/deploy-app.sh monensure
        """
      }
      post {
        always {
